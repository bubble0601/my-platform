      ...
      <template #header>
        <template v-if="$pc">
          <!-- <v-sheet class="d-flex align-center px-2 py-1">
            ...
            <template v-if="internalSelecting">
            ...
            <v-tooltip v-if="!internalSelecting" bottom open-delay="1000">
              <template #activator="{ on, attrs }">
        ...
        <template v-else>
          <!-- <v-sheet class="d-flex align-center pa-1">
            ...
            <v-menu v-if="internalSelecting" tile bottom offset-y>
              <template #activator="{ on, attrs }">
                <v-btn v-bind="attrs" small plain text v-on="on">
                  <v-icon>playlist_add</v-icon>
                  <span class="ml-1">プレイリストに追加</span>
...
export default class SongList extends Vue {
  ...
  private internalSelecting = false;
  ...
  get selectData() {
    return {
      enabled: this.internalSelecting,
  ...
  // }
  @Watch('selecting')
  private onSelectingChanged(val: boolean) {
    this.internalSelecting = val;
    if (!val) {
      this.selectedSongs = [];
    }
  }
  @Watch('internalSelecting', { immediate: true })
  private onInternalSelectingChanged(val: boolean) {
    this.$emit('update:selecting', val);
  }
  @Watch('value')
  private onValueChanged(val: Song[]) {
    this.selectedSongs = val;
  }
  @Watch('selectedSongs', { immediate: true })
  private onSelectedSongsChanged(val: Song[]) {
    this.$emit('input', val);
    if (!val.length) {
  ...
  protected created() {
    this.sortBy = this.context === 'artist' ? 'year' : 'artist.name';
  }

  private select(song: Song, add: boolean) {
    if (add) {
