<template>
  <div v-ripple="$mobile" class="player py-1" @click="expand">
    <audio
      ref="audio"
      :src="audioSrc"
    ...
    <v-progress-linear v-if="$mobile" :value="progress100" absolute top/>

    <div class="d-flex align-center w-100 h-100" :class="{ 'px-4': $pc }">
      <div class="song-info">
        <cover-art/>
      ...
      <v-btn :loading="loading" x-large icon v-on="stopRipple" @click.stop="playing = !playing">
      ...
      <template v-if="$pc">
        <v-slider :disabled="audioSrc === null" :value="progress" :max="max" thumb-label thumb-size="20px" hide-details class="px-2" @change="seek">
          <template #prepend>
    ...
    <v-dialog v-if="$mobile" v-model="mobileExpanded" fullscreen transition="dialog-bottom-transition" overlay-opacity="0">
      <v-card class="d-flex flex-column h-100">
        <v-toolbar flat color="transparent" class="flex-grow-0" v-touch="{ down: () => mobileExpanded = false }">
          <v-btn icon @click="mobileExpanded = false">
