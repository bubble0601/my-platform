...
function calculateNodeStyling(targetElement: HTMLTextAreaElement) {
  ...
  const contextStyle = CONTEXT_STYLE
    .map((name: string) => `${name}:${style.getPropertyValue(name)}`)
    .join(';');

  return { contextStyle, paddingSize, borderSize, boxSizing };
}

export function calcTextareaHeight(targetElement: HTMLTextAreaElement, minRows = 1, maxRows: number | null = null) {
  if (!hiddenTextarea) {
...
export function calcElementHeight(targetElement: HTMLTextAreaElement) {
